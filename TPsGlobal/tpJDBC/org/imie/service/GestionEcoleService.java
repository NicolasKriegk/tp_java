...
public class GestionEcoleService implements IGestionEcoleService,IJDBCCompatible {

	//CDI
//	@Inject
//	@TransactionBeanJDBC(TransactionModeJDBC.THREADSAFE) 
	//EJB
	@EJB(beanName="PersonneDAO")
	private IPersonneDAO personneDAO = null;
	//CDI
//	@Inject
//	@TransactionBeanJDBC(TransactionModeJDBC.THREADSAFE) 
	//EJB
	@EJB(beanName="PromotionDAO")
	private IPromotionDAO promotionDAO = null;
	private Connection connection;

	//pour le CDI
	public GestionEcoleService() {
	}
	...
	public void deletePromotion(Promotion promotion) throws ServiceException {
		try {
			...
			((IJDBCCompatible) personneDAO).beginTransaction(this);
			...
			((IJDBCCompatible) personneDAO).endTransaction(exception);

			((PersonneDAO) promotionDAO).beginTransaction(this);
			try {
				promotionDAO.deletePromotion(promotion);
			} catch (PersistanceException e) {
				exception = e;
			}
			((PersonneDAO) promotionDAO).endTransaction(exception);
