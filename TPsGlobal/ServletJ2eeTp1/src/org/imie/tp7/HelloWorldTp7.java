 ...
 */
@WebServlet(
		urlPatterns = { "/HelloWorldTp7" }, 
		initParams = { 
				@WebInitParam(name = "design", value = "rouge"),
		})
public class HelloWorldTp7 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("redirect".equals(request.getParameter("mode"))){
			response.sendRedirect("./TableauTp7");
		}
		else if("forward".equals(request.getParameter("mode"))){
			ServletContext context= request.getSession().getServletContext();
			RequestDispatcher rd = context.getRequestDispatcher("/TableauTp7");
			...
			ServletContext context= request.getSession().getServletContext();
			RequestDispatcher rd= context.getRequestDispatcher("/TableauTp7Include");
