 ...
 */
@WebServlet(
		urlPatterns = { "/HelloWorldTp6" }, 
		initParams = { 
				@WebInitParam(name = "design", value = "rouge"),
		})
public class HelloWorldTp6 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("redirect".equals(request.getParameter("mode"))){
			response.sendRedirect("./TableauTp6");
		}
		else if("forward".equals(request.getParameter("mode"))){
			ServletContext context= request.getSession().getServletContext();
			RequestDispatcher rd = context.getRequestDispatcher("/TableauTp6");
			...
			ServletContext context= request.getSession().getServletContext();
			RequestDispatcher rd= context.getRequestDispatcher("/TableauTp6Include");
